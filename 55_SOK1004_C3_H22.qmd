...
df <- df %>%                                  #Omskriver df og skifter navn på
  rename("vekt" = "value") %>%                #value til vekt og fjerner year month
...
oppgave_3a <- df %>%    
  filter(statistikkvariabel == "Konsumprisindeks (vekter)") %>% #Henter bare vekter
  mutate(vekt = vekt/1000)  %>%      #deler vekt på 1000 for å sjekke avvik
  group_by(dato) %>%              #grupperer med dato
  summarize(sjekk <- sum(vekt))     #summer opp beløpene på dato.
...
df_total <- df_vekter %>% 
  cbind(df_index$index) %>% 
  mutate(indeks = df_index$index*vekt) %>% 
  select(-statistikkvariabel)


df_sum <- df_total %>%
  group_by(dato) %>% 
  summarize(index = sum(indeks, na.rm=TRUE))

df_sum %>%
  ggplot(aes(x=dato, y=index))+
  geom_line() +
  theme_bw()
